...
$(function() {
	errCodeCheck();
	// 사용자에게 요구사항에 대한 문자열로 배열초기화
	var message = [ "영문, 숫자만 가능. 6~12자로 입력해주세요",
			"영문 숫자, 특수문자만 가능. 8~15자로 입력해주세요", "비밀번호와 비밀번호 확인란은 값이 일치해야 합니다.",
	...
	$("#idConfirmBtn").click(function(){
		if(!fromCheck($('#userId'), $('error:eq(0)'), "아이디를")){
			return;
			...
			$.ajax({
				...
				sucess: function(resultData){
					...
					if(resultData=="1"){
						...
						$("#userId").parents(".from-group").find(".error").html("사용 가능한 아이디입니다.");
						idConfirm=2;
					}
				}
			});
	...
	$("#joinInsert").click(function(){
		...
		if(!formCheck($('#userId'), $('.error:eq(0)'), "아이디를")){
			...
			return;
		} else if(!formCheck($('#emailName'), $('.error:eq(3)'), "이메일 주소를")){
			return;
		} else if(idConfirm!=2){
